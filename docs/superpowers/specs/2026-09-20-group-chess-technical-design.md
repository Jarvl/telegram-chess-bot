...
5. `BackButton` is bound to the router and always means "go back", never "close" — Telegram supplies its own close control at the root. A deep link seeds the screens above the one it names (`g_<gameId>` → Groups, Lobby, Game; `l_<groupId>` → Groups, Lobby; `s_<groupId>` → Groups, Lobby, Group settings), so a player who tapped a card can reach that group's games and their other groups without returning to the chat to reopen the app. `close()` stays on the paths that mean it: return-to-chat after a move, and **Done** on a finished game.
